...
const enableGa =
  process.env.NODE_ENV === 'production' || process.env.NUXT_PUBLIC_ENABLE_GA === 'true'
...
export default defineNuxtConfig({
  ...
  pwa: {
    ...
    workbox: {
      ...
      // PWA 離線時導向 public/offline.html；含「重試」按鈕與線上偵測
      navigateFallback: '/offline.html',
      navigateFallbackDenylist: [/^\/api\//, /^\/sitemap\.xml/, /^\/feed\.xml/, /^\/robots\.txt/],
  ...
  nitro: {
    prerender: {
      routes: ['/about', '/faq', '/qs', '/compare', '/calculator']
    }
  },

  // #23 全站 CSP（含必要的第三方白名單：GTM / GA / Cloudflare Insights / LINE LIFF /
  // Supabase / wsrv.nl 圖床 / GitHub Raw / jsDelivr）。
  // 'unsafe-inline' 保留：站內大量 JSON-LD 與 inline style；'unsafe-eval' 給 Pinia / GSAP / Vue 編譯期使用
  routeRules: {
    '/**': {
      headers: {
        'Content-Security-Policy': [
          "default-src 'self'",
          "script-src 'self' 'unsafe-inline' 'unsafe-eval' https://www.googletagmanager.com https://www.google-analytics.com https://static.cloudflareinsights.com https://static.line-scdn.net",
  ...
  // url / key 由 @nuxtjs/supabase 從 env 自動讀（SUPABASE_URL / SUPABASE_KEY）
  // 本機：.env；Vercel：Project Settings → Environment Variables
  supabase: {
    redirect: false
  },
    ...
    head: {
      ...
      meta: [
        ...
        },
        {
          name: 'keywords',
          content:
            '豹紋守宮,守宮,基因計算機,守宮品系,守宮飼養,新手飼養,Eublepharis macularius,Gencko,捷客工作室,守宮繁育,守宮選購,特寵醫院'
        },
      ...
      link: [
        ...
        {
          rel: 'icon',
          href: 'https://cdn.jsdelivr.net/gh/zzes50708/gencko-assets@main/img/11.png'
        },
        {
        ...
        // #5 字型精簡：移除 Noto Sans TC weight 300（全站僅 6 處使用，瀏覽器從 400 合成肉眼無感）
        {
          rel: 'preload',
          as: 'style',
      ...
      script: [
        ...
        // Organization 結構化資料（品牌身份，GEO / Knowledge Panel 基礎）
        {
          ...
          innerHTML: JSON.stringify({
            ...
            description:
              'Gencko Breeding Studio 是台灣專業的豹紋守宮（Eublepharis macularius）繁育工作室，提供特殊基因品系個體選購、基因計算機、新手飼養教學、健康評估、特寵醫院查詢與線上競標。從基因到飼養的一站式服務。',
            ...
            hasOfferCatalog: {
              ...
              itemListElement: [
                ...
                },
                {
                  '@type': 'Offer',
                  name: '線上競標',
                  url: 'https://www.genckobreeding.com/auction'
                },
                {
                  '@type': 'Offer',
                  name: '健康評估',
                  url: 'https://www.genckobreeding.com/health'
                },
                { '@type': 'Offer', name: '飼養前評估', url: 'https://www.genckobreeding.com/qs' },
                {
                  '@type': 'Offer',
                  name: '特寵醫院查詢',
                  url: 'https://www.genckobreeding.com/hospital'
                }
              ]
            },
            sameAs: [
