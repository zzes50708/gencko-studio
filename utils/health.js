...
export const VERDICTS = {
    ...
    emergency: {
        ...
        action: '前往醫院前請保溫（25–28°C）、避免劇烈搖晃、不要灌食灌水、攜帶近 3 天糞便照片或實物樣本。'
    }
}
...
export const TRIAGE_QUESTIONS = [
    {
        ...
        type: 'single',
        options: [
            { id: 'seizure',    label: '抽搐、痙攣、突然無法翻身',                                                  severity: 'critical', diseases: ['neuroToxic', 'mbd', 'mystery', 'kidneyFailure'] },
            { id: 'breathing',  label: '大口張嘴吸氣 / 頻繁打哈欠樣動作（可能是呼吸困難而非正常打呵欠）',           severity: 'critical', diseases: ['ri', 'heatExhaust'] },
            { id: 'mouth',      label: '嘴角牽絲黏液 / 鼻孔阻塞或結痂（不是剛喝完水的水珠）',                       severity: 'critical', diseases: ['ri'] },
            { id: 'vomit',      label: '嘔吐 / 反芻已消化的食物（不只是吐出剛吃進的）',                              severity: 'critical', diseases: ['giStasis', 'overfeeding', 'crypto', 'impaction'] },
            { id: 'prolapse',   label: '泄殖腔有粉紅色組織外露 / 半陰莖無法縮回（不是正常排便突起）',                severity: 'critical', diseases: ['prolapse'] },
            { id: 'paralysis',  label: '四肢完全癱軟、長時間貼地拖行（不是放鬆休息）',                              severity: 'critical', diseases: ['mbd', 'neuroToxic'] },
    ...
    },
    {
        ...
        options: [
            ...
            { id: 'noStool',        label: '5–7 天內沒排便但腹部正常、活動進食正常',                severity: 'normal',
                normalReason: '脫皮前、低溫期、剛吃飽都可能 5–7 天無便，屬正常變異。' },
            { id: 'softOdorless',   label: '偶爾偏軟、無異味',                                       severity: 'watch' },
            { id: 'runnySmell',     label: '持續拉稀、顏色正常但極臭',                                severity: 'warn',     diseases: ['coccidia', 'crypto', 'bacterialEnteritis'] },
            { id: 'bloated',        label: '腹部明顯脹大 / 超過 7 天無便 / 無精神',                  severity: 'critical', diseases: ['impaction', 'eggBinding'],
                normalReason: '母成體必須懷疑卡蛋 / 卵泡留滯。' },
            { id: 'blood',          label: '糞便帶血絲、鮮血、深紅色',                                severity: 'critical', diseases: ['lowerGiBleeding'] },
            { id: 'wateryVomit',    label: '水樣便 + 腥臭 + 反覆吐食',                                severity: 'critical', diseases: ['crypto'] },
            { id: 'yellowUrate',    label: '尿酸從白變鮮黃或橘紅色',                                  severity: 'warn',     diseases: ['dehydration', 'kidneyFailure'],
    ...
    },
    {
        id: 'gender',
        title: 'Q3-1. 性別與年齡',
        subtitle: '影響拒食判讀。幼亞成體拒食 1–2 週以上極度危險；成年母體春季拒食大多是發情排卵（即使未交配也可能空包蛋）；成年公體繁殖期躁動拒食屬正常。',
        type: 'single',
        options: [
            { id: 'maleAdult',   label: '公成體（1 歲以上）',     severity: 'normal',
                normalReason: '繁殖期（1–4 月）躁動拒食屬正常現象。' },
            { id: 'femaleAdult', label: '母成體（1 歲以上）',     severity: 'normal',
                normalReason: '春天拒食大多為發情排卵，即使未交配也可能空包蛋，未處理會導致卵泡留滯（卡蛋）。' },
            { id: 'subadult',   label: '亞成體（6–12 個月）',     severity: 'normal',
                normalReason: '亞成體代謝快，拒食超過 1–2 週極度危險。' },
            { id: 'juvenile',   label: '幼體（< 6 個月）',         severity: 'normal',
    ...
    },
    {
        ...
        options: [
            ...
            { id: 'd14', label: '1–2 週',                  severity: 'warn',     diseases: ['parasites', 'organFailure', 'eggBinding'] },
            { id: 'd30', label: '2 週至 1 個月',           severity: 'warn',     diseases: ['parasites', 'organFailure', 'eggBinding'] },
            { id: 'd60', label: '超過 1 個月',             severity: 'critical', diseases: ['parasites', 'organFailure', 'eggBinding'] }
        ]
    },
    {
        id: 'fastSymptoms',
        ...
        type: 'multi',
        options: [
            { id: 'weightLoss',  label: '體重明顯下降（拿起來變輕）',                severity: 'warn',     diseases: ['crypto'],
                normalReason: '建議從現在開始每週量一次體重，若 1 個月內掉 10% 以上請就醫。' },
            { id: 'vomiting',    label: '嘔吐或反芻',                                  severity: 'critical', diseases: ['crypto'] },
            { id: 'hidingAll',   label: '整天躲洞、碰也無反應',                        severity: 'warn',     diseases: ['parasites', 'organFailure', 'eggBinding'] },
            { id: 'pencilTail',  label: '尾巴明顯變細（牙籤尾）',                     severity: 'critical', diseases: ['crypto', 'malnutrition', 'pinworm', 'chronicDehydration'] },
    ...
    },
    {
        ...
        options: [
            ...
            { id: 'thinOk',   label: '略偏瘦但活動進食正常',                            severity: 'watch',
                normalReason: '剛產卵母守宮、剛斷尾再生中、發情公守宮短期可能略瘦。' },
            { id: 'thinFast', label: '1 個月內明顯瘦了一圈',                            severity: 'warn',     diseases: ['parasites', 'anorexia', 'malnutrition'] },
            { id: 'pencil',   label: '「牙籤尾」皮包骨',                                severity: 'critical', diseases: ['crypto', 'coccidia', 'parasites', 'anorexia', 'malnutrition'] }
        ]
    },
    {
        id: 'behavior',
        ...
        options: [
            ...
            { id: 'preShed', label: '比平常安靜 + 體色變暗',                            severity: 'normal',
                normalReason: '脫皮前 1–3 天典型徵兆。' },
            { id: 'slow',    label: '比平常慢但仍會出來進食',                           severity: 'watch' },
            { id: 'hiding',  label: '整天躲洞 + 拒食 + 拿出來也不太掙扎',              severity: 'warn',     diseases: ['parasites', 'anorexia', 'malnutrition'] },
            { id: 'crash',   label: '異常活躍、不停撞缸壁、繞圈打轉',                  severity: 'critical', diseases: ['neuroToxic'] }
        ]
    },
    {
        id: 'env',
        ...
        type: 'multi',
        options: [
            { id: 'newEnv',     label: '24 小時內換新環境 / 搬家 / 剛買回家',          severity: 'normal',
                normalReason: '壓力期，輕微異常先觀察 1–2 週。' },
            { id: 'tempBad',    label: '最近 3 天溫度低於 22°C 或高於 35°C',           severity: 'warn',     diseases: ['tempAbnormal', 'giStasis', 'heatExhaust'],
                normalReason: '高危險：低溫導致腸胃停滯，高溫導致熱衰竭。' },
            { id: 'contagion',  label: '最近接觸 / 同缸其他守宮（傳染風險）',           severity: 'watch',    diseases: ['contagionRisk'] },
            { id: 'substrate',  label: '飼主換過底材（疑似底材誤食 / 中毒）',          severity: 'watch',    diseases: ['impaction'] },
...
export const CHECKUP_QUESTIONS = [
    // A. 頭部與五官
    {
        id: 'eyes', category: '頭部與五官', title: 'A1. 眼睛狀態', type: 'single',
        options: [
            { id: 'clear',    label: '雙眼大睜、清澈、無分泌物',           severity: 'normal' },
            { id: 'halfShut', label: '一眼半閉（剛醒 / 睡眠中可能）',      severity: 'watch',     diseases: ['eyeIssue'] },
            { id: 'shedRes',  label: '眼周有殘餘脫皮',                       severity: 'watch',     diseases: ['dysecdysis'] },
            { id: 'sunken',   label: '眼睛凹陷、皮膚皺褶',                   severity: 'warn',      diseases: ['dehydration'] },
            { id: 'discharge', label: '眼瞼內積聚黃白色固體物 / 眼球表面卡白膜', severity: 'warn',  diseases: ['eyeIssue'] },
            { id: 'shutLong', label: '完全閉眼超過 24 小時（非脫皮）',       severity: 'critical',  diseases: ['eyeIssue'] }
        ]
    },
    {
        id: 'nose', category: '頭部與五官', title: 'A2. 鼻孔狀態', type: 'single',
        options: [
    ...
    {
        id: 'mouth', category: '頭部與五官', title: 'A3. 口腔狀態', type: 'single',
        options: [
            ...
            { id: 'foamSwell',  label: '口腔邊緣膿塊 / 嚴重腫脹 / 嘴歪斜',       severity: 'critical', diseases: ['stomatitis'] },
            { id: 'openMouth',  label: '長時間張口呼吸',                       severity: 'critical', diseases: ['ri'] }
        ]
    },
    {
        id: 'breathing', category: '頭部與五官', title: 'A4. 呼吸狀態', type: 'single',
        options: [
            ...
            { id: 'bubble',    label: '張口呼吸 / 啵啵聲 / 嘴角牽絲黏液',       severity: 'critical', diseases: ['ri'] }
        ]
    },
    {
        id: 'jaw', category: '頭部與五官', title: 'A5. 頭部與下顎外觀', type: 'single',
        options: [
            { id: 'symmetry',  label: '左右對稱、下顎線條正常',                 severity: 'normal' },
            { id: 'oldScar',   label: '有舊疤但無腫脹、無變形',                 severity: 'watch' },
            { id: 'swollen',   label: '臉頰或嘴角單側腫起、下顎變厚',           severity: 'warn',     diseases: ['stomatitis'] },
            { id: 'softJaw',   label: '下顎軟塌 / 歪斜 / 無法正常閉嘴',         severity: 'critical', diseases: ['mbd', 'stomatitis'] }
        ]
    },
    {
        id: 'tongue', category: '頭部與五官', title: 'A6. 舌頭與口腔內部顏色', type: 'single',
        ...
        options: [
            ...
            { id: 'cyanotic',  label: '舌頭或黏膜偏藍紫色 / 紫紅',              severity: 'critical', diseases: ['severeHypoxia', 'sepsisTerminal'] }
        ]
    },
    // B. 體表與脫皮
    {
        id: 'skin', category: '體表與脫皮', title: 'B1. 皮膚完整度', type: 'single',
        options: [
            { id: 'intact',   label: '光滑完整無傷',                          severity: 'normal' },
            { id: 'oldScar',  label: '有舊傷疤（已癒合）',                     severity: 'normal' },
            { id: 'scratch',  label: '抓痕 / 咬傷 / 殘皮卡 > 1 週',           severity: 'warn',     diseases: ['dysecdysis'] },
            { id: 'burn',     label: '燒燙傷（腹部黑斑）',                     severity: 'critical', diseases: ['burn'] },
            { id: 'wound',    label: '開放傷口 / 潰瘍 / 組織壞死',             severity: 'critical', diseases: ['burn'] }
        ]
    },
    {
        id: 'shed', category: '體表與脫皮', title: 'B2. 脫皮狀態', type: 'single',
        options: [
            ...
            { id: 'tailTip',  label: '尾尖殘皮（壞死斷尾風險）',                severity: 'warn',     diseases: ['dysecdysis'] },
            { id: 'chronic',  label: '反覆脫皮不順、體色長期暗沉',              severity: 'warn',     diseases: ['dysecdysis'] }
        ]
    },
    {
        id: 'color', category: '體表與脫皮', title: 'B3. 體色', type: 'single',
        options: [
            { id: 'vivid',   label: '顏色飽和明亮（依品系判斷）',              severity: 'normal' },
            { id: 'preShed', label: '整體略暗（脫皮前 1–3 天）',                severity: 'normal' },
            { id: 'dullLong', label: '持續暗沉 > 1 週、無脫皮跡象',             severity: 'watch',    diseases: ['metabolicStasis', 'envStress'] },
            { id: 'pale',    label: '異常蒼白、灰白色',                         severity: 'critical', diseases: ['severeAnemia', 'preShock', 'sepsis'] }
        ]
    },
    {
        id: 'toesTail', category: '體表與脫皮', title: 'B4. 趾尖與尾尖狀態', type: 'single',
        options: [
            { id: 'clean',      label: '趾尖、尾尖乾淨無殘皮',                  severity: 'normal' },
            { id: 'smallFlake', label: '少量碎皮，2–3 天內可自行脫落',          severity: 'watch' },
            { id: 'stuck',      label: '趾尖或尾尖有卡皮、環狀束住',            severity: 'warn',     diseases: ['dysecdysis'] },
            { id: 'necrosis',   label: '趾尖發黑 / 尾尖乾縮 / 疑似壞死',        severity: 'critical', diseases: ['dysecdysis'] }
        ]
    },
    {
        id: 'vent', category: '體表與脫皮', title: 'B5. 腹部與泄殖腔周圍', type: 'single',
        options: [
            { id: 'clean',      label: '腹部與泄殖腔乾淨、無殘便無紅腫',        severity: 'normal' },
            { id: 'stain',      label: '剛排便後有少量污漬，但可自行清掉',      severity: 'watch' },
            { id: 'irritated',  label: '泄殖腔周圍反覆沾便、紅腫或殘皮',        severity: 'warn',     diseases: ['dysecdysis'] },
            { id: 'prolapse',   label: '外翻 / 出血 / 明顯腫塊',                severity: 'critical', diseases: ['prolapse'] }
        ]
    },
    {
        id: 'limbs', category: '體表與脫皮', title: 'B6. 四肢、趾數與趾甲', type: 'single',
        ...
        options: [
            ...
            { id: 'freshLost',  label: '近期缺趾 / 傷口未癒合',                  severity: 'warn',     diseases: ['dysecdysis'] },
            { id: 'necrotic',   label: '趾尖或四肢局部發黑壞死、流組織液',       severity: 'critical', diseases: ['dysecdysis', 'burn'] }
        ]
    },
    // C. 體態與進食
    {
        id: 'tail', category: '體態與進食', title: 'C1. 尾巴體態', type: 'single',
        options: [
            ...
            { id: 'pencil',  label: '牙籤尾（強烈警訊）',                       severity: 'critical', diseases: ['crypto', 'coccidia'] }
        ]
    },
    {
        id: 'weight', category: '體態與進食', title: 'C2. 體重變化（若無紀錄請選最後一項）', type: 'single',
        options: [
            { id: 'stable',   label: '穩定（± 5%）',                            severity: 'normal' },
            { id: 'gain',     label: '緩步增重（幼亞成體正常）',                severity: 'normal' },
            { id: 'lose10',   label: '1 個月內掉 5–10%',                        severity: 'watch' },
            { id: 'lose20',   label: '1 個月內掉 10–20%',                       severity: 'warn',     diseases: ['parasites', 'anorexia'] },
            { id: 'lose30',   label: '1 個月內掉 20% 以上 / 任何時段急速消瘦', severity: 'critical', diseases: ['crypto'] },
            { id: 'noData',   label: '沒量過 / 沒紀錄',                          severity: 'watch' }
        ]
    },
    {
        id: 'appetite', category: '體態與進食', title: 'C3. 食慾', type: 'single',
        options: [
            { id: 'normal',     label: '正常進食、看到飼料會追',                severity: 'normal' },
            { id: 'reduced',    label: '進食量比平常少 1/3，仍會吃',             severity: 'watch' },
            { id: 'spit',       label: '反覆甩頭吐食 / 含一下就吐',              severity: 'critical', diseases: ['crypto'] },
            { id: 'refuse',     label: '完全拒食 + 體態消瘦',                    severity: 'critical', diseases: ['organFailure', 'severeParasites', 'eggBinding'] },
            { id: 'maleSeason', label: '公成體春季完全拒食 + 其他指標正常',     severity: 'normal',
                normalReason: '發情期拒食，多數會在 1–2 個月後恢復。' },
            { id: 'newAdjust',  label: '新個體入手 2 週內拒食 + 其他指標正常',  severity: 'normal',
                normalReason: '環境適應期，建議保持安靜減少干擾。' }
        ]
    },
    {
        id: 'bodyLine', category: '體態與進食', title: 'C4. 軀幹線條與骨感', type: 'single',
        options: [
            { id: 'solid',     label: '背線平順、肋骨與骨盆不明顯',             severity: 'normal' },
            { id: 'slim',      label: '略瘦但肌肉仍在、腹部不凹',               severity: 'watch' },
            { id: 'bony',      label: '骨盆、脊椎、肋骨變明顯',                 severity: 'warn',     diseases: ['parasites', 'organFailure', 'eggBinding'] },
            { id: 'wasted',    label: '全身快速消瘦、明顯皮包骨',               severity: 'critical', diseases: ['crypto'] }
        ]
    },
    {
        id: 'hydration', category: '體態與進食', title: 'C5. 水分狀態', type: 'single',
        options: [
            { id: 'good',      label: '眼神正常、皮膚平整、尿酸多為白色',       severity: 'normal' },
            { id: 'unsure',    label: '沒有特別觀察過喝水與尿酸狀態',           severity: 'watch' },
            { id: 'dry',       label: '尿酸偏黃、皮膚略皺、精神下降',           severity: 'warn',     diseases: ['dehydration'] },
            { id: 'dehydrated', label: '眼窩深陷、皮膚皺褶明顯、長期不喝水',    severity: 'critical', diseases: ['dehydration'] }
        ]
    },
    {
        id: 'abdomen', category: '體態與進食', title: 'C6. 腹部外觀', type: 'single',
        options: [
            { id: 'flat',       label: '腹部平順、不鼓脹、不拖地',               severity: 'normal' },
            { id: 'afterMeal',  label: '進食後短暫微鼓，數小時內恢復',           severity: 'watch' },
            { id: 'bloated',    label: '長時間腹脹、排便變少',                   severity: 'warn',     diseases: ['eggBinding', 'tumor', 'constipation', 'impaction'] },
            { id: 'mass',       label: '明顯腫塊 / 單側鼓起 / 腹部拖地',         severity: 'critical', diseases: ['eggBinding', 'tumor', 'severeConstipation', 'impaction'] }
        ]
    },
    {
        id: 'feederVariety', category: '體態與進食', title: 'C7. 飼料多樣性', type: 'single',
        ...
        options: [
            ...
            { id: 'mealwormOnly', label: '只餵麵包蟲 / 大麥蟲長期單一',          severity: 'warn',     diseases: ['nutritionalImbalance'] },
            { id: 'sweets',     label: '長期以蠟蟲 / 大麥蟲為主食（高脂）',       severity: 'warn',     diseases: ['fattyLiver', 'calciumPhosphorus', 'mbd'] },
            { id: 'unknown',    label: '飼料來源不明 / 野外捕捉昆蟲',             severity: 'warn',     diseases: ['coccidia', 'crypto'] }
        ]
    },
    {
        id: 'drinking', category: '體態與進食', title: 'C8. 飲水習慣', type: 'single',
        options: [
            ...
            { id: 'none',       label: '長期不喝水 + 尿酸偏黃 / 偏少',            severity: 'warn',     diseases: ['dehydration'] }
        ]
    },
    // D. 排泄行為環境
    {
        id: 'droppings', category: '排泄行為環境', title: 'D1. 糞便與排泄', type: 'single',
        options: [
            ...
            { id: 'blood',    label: '血便 / 反覆吐食 + 水便',                 severity: 'critical', diseases: ['crypto', 'impaction'] },
            { id: 'urate',    label: '尿酸鮮黃或橘紅（脫水）',                  severity: 'warn',     diseases: ['dehydration'] }
        ]
    },
    {
        id: 'mbd', category: '排泄行為環境', title: 'D2. 行動穩定度（MBD 篩查）', type: 'single',
        options: [
            ...
            { id: 'hop',      label: '跳走、四肢無力、下顎軟塌、骨頭彎曲',       severity: 'critical', diseases: ['mbd'] },
            { id: 'seizure',  label: '抽搐 / 痙攣',                              severity: 'critical', diseases: ['neuroToxic', 'mbd'] }
        ]
    },
    {
        id: 'envCare', category: '排泄行為環境', title: 'D3. 環境溫控與補充', type: 'single',
        options: [
            { id: 'full',    label: '完整溫度梯度 + 規律鈣粉每餐',                severity: 'normal' },
            { id: 'noTherm', label: '有加溫但無控溫器（燙傷風險）',                severity: 'warn',  diseases: ['burn'] },
            { id: 'noMeas',  label: '沒測量過溫度 / 全缸同溫',                    severity: 'warn',  diseases: ['detectionFail'] },
            { id: 'none',    label: '完全沒溫控 + 沒補鈣粉',                       severity: 'critical', diseases: ['mbd'] }
        ]
    },
    {
        id: 'activity', category: '排泄行為環境', title: 'D4. 活動力與警覺性', type: 'single',
        options: [
            ...
            { id: 'collapsed',  label: '翻正困難 / 幾乎不動 / 無力撐身',         severity: 'critical', diseases: ['neuroTerminal'] }
        ]
    },
    {
        id: 'stance', category: '排泄行為環境', title: 'D5. 站姿與抓地力', type: 'single',
        options: [
    ...
    {
        id: 'setup', category: '排泄行為環境', title: 'D6. 躲避屋與濕盒配置', type: 'single',
        options: [
            { id: 'complete',   label: '冷區、熱區、濕盒齊全且可正常使用',       severity: 'normal' },
            { id: 'partial',    label: '有躲避屋，但濕盒偶爾才補濕',             severity: 'watch' },
            { id: 'missingOne', label: '缺濕盒或長期太乾，脫皮常卡',             severity: 'warn',     diseases: ['dysecdysis'] },
            { id: 'bare',       label: '幾乎無躲避設施，環境長期失衡',           severity: 'critical' }
        ]
    },
    {
        id: 'records', category: '排泄行為環境', title: 'D7. 體重、進食與糞檢紀錄', type: 'single',
        options: [
            { id: 'tracked',    label: '有固定記錄，必要時也有做糞檢',           severity: 'normal' },
            { id: 'partial',    label: '只有部分記錄，頻率不固定',               severity: 'watch' },
            { id: 'none',       label: '幾乎沒記錄，最近又出現軟便或掉重',       severity: 'warn',     diseases: ['coccidia'] },
            { id: 'chronic',    label: '長期腹瀉 / 消瘦，且從未檢糞',            severity: 'critical', diseases: ['crypto', 'coccidia'] }
        ]
    },
    {
        id: 'humidity', category: '排泄行為環境', title: 'D8. 環境濕度', type: 'single',
        ...
        options: [
            ...
            { id: 'tooDry',     label: '長期 < 30% 且無濕盒，脫皮反覆失敗',      severity: 'warn',     diseases: ['dysecdysis', 'dehydration'] },
            { id: 'tooWet',     label: '長期 > 70% 滿缸潮濕、底材長黴',           severity: 'warn',     diseases: ['ri', 'bacterialDermatitis'] },
            { id: 'noMeasure',  label: '從未測過濕度 / 沒有溫濕度計',             severity: 'watch' }
        ]
    },
    {
        id: 'calcium', category: '排泄行為環境', title: 'D9. 鈣粉與綜合維生素補充', type: 'single',
    ...
    {
        id: 'cohab', category: '排泄行為環境', title: 'D10. 同箱個體狀況', type: 'single',
        subtitle: '⚠️ 守宮禁止任何形式的混養，若有混養情況，請立即停止，堅持一缸一體。',
        options: [
            { id: 'solo',       label: '單獨飼養',                                 severity: 'normal' },
            { id: 'femalePair', label: '母 + 母混養，無爭執且皆健康',              severity: 'critical', diseases: ['chronicStress'] },
            { id: 'malePair',   label: '兩隻成熟公守宮同箱 / 有打鬥傷口',          severity: 'critical', diseases: ['severeStress', 'woundInfection'] },
            { id: 'sizeGap',    label: '混養但體型差距大 / 弱勢個體常被搶食',      severity: 'warn',     diseases: ['malnutrition', 'limbLossRisk', 'chronicStress'] }
        ]
    }
]
...
export const PURCHASE_QUESTIONS = [
    {
        id: 'eyes', title: 'Q1. 眼睛', type: 'single',
        options: [
            { id: 'clear',    label: '雙眼大睜、清澈、無分泌物',                  severity: 'normal' },
    ...
    {
        id: 'body', title: 'Q2. 體態與尾巴', type: 'single',
        options: [
            { id: 'plump',    label: '飽滿圓潤、尾巴比例正常',                      severity: 'normal' },
            { id: 'okThin',   label: '略瘦但仍有肉',                                 severity: 'watch' },
            { id: 'thin',     label: '明顯偏瘦、脊椎可見',                            severity: 'warn' },
            { id: 'pencil',   label: '牙籤尾（皮包骨，🚨 deal-breaker）',           severity: 'critical', diseases: ['crypto'] }
        ]
    },
    {
        id: 'skin', title: 'Q3. 皮膚與脫皮', type: 'single',
        options: [
            { id: 'intact',  label: '光滑完整無傷、無殘皮',                          severity: 'normal' },
            { id: 'minor',   label: '少量殘皮可協助移除',                              severity: 'watch' },
            { id: 'wound',   label: '殘皮卡關節 / 咬痕 / 抓傷',                        severity: 'warn',     diseases: ['dysecdysis'] },
            { id: 'burn',    label: '燒燙傷痕跡 / 潰瘍 / 組織壞死 / 大面積傷口',       severity: 'critical', diseases: ['burn'] }
        ]
    },
    {
        id: 'response', title: 'Q4. 精神與神經狀態', type: 'single',
        options: [
            { id: 'alert',    label: '眼神明亮，把手靠近會盯著看、會舔鼻子，上手會試圖爬行或輕微掙扎', severity: 'normal' },
            { id: 'tremor',   label: '上手時前肢一直發抖 / 走路腹部拖地',                              severity: 'warn',     diseases: ['earlyMbd', 'neuroDamage'] },
            { id: 'wobble',   label: '仰頭看天（星際觀望）/ 原地轉圈圈 / 瘋狂撲空',                    severity: 'critical', diseases: ['mystery'] },
            { id: 'limp',     label: '放在手上像一灘爛泥完全不動、眼睛閉著',                            severity: 'critical', diseases: ['severeWeakness'] }
        ]
    }
]
