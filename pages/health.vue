  ...
  step: [
    {
      ...
      position: 1,
      name: '選擇情境',
      text: '從入口頁三張卡片選擇：① 緊急快篩（飼主察覺異常需快速判斷是否就醫）② 完整健康檢查（定期健康紀錄）③ 購入評估（購買前/取貨時檢查個體狀況）。'
    },
    {
      '@type': 'HowToStep',
      position: 2,
      name: '依序回答問題',
      text: '依問卷引導逐題作答，題目依當下情境動態調整。所有作答僅儲存在本機 sessionStorage，不上傳。'
    },
    {
      '@type': 'HowToStep',
      position: 3,
      name: '查看判讀結果',
      text: '完成後系統顯示綜合判讀：是否觸發立即就醫警訊、可能的正常變異解釋、可能狀況（疾病比對）、各項目逐題判讀，並依嚴重度標示顏色。'
    },
    {
      '@type': 'HowToStep',
      position: 4,
      name: '前往特寵醫院或截圖給獸醫',
      text: '若觸發就醫警訊，可一鍵跳到 /hospital 查詢附近特寵醫院；或截圖「給獸醫看的報告」區，掛號時直接給醫師參考。'
    }
  ]
}
  ...
  mainEntity: [
    {
      ...
      acceptedAnswer: {
        '@type': 'Answer',
        text: '不行。本評估系統僅供初步判斷與飼主自我檢視，無法取代專業獸醫的臨床診斷、影像檢查或實驗室檢驗。當系統觸發「立即就醫警訊」時，請盡快帶守宮到專業的特寵醫院（可使用 /hospital 查詢）。'
      }
    },
    {
      '@type': 'Question',
      ...
      acceptedAnswer: {
        '@type': 'Answer',
        text: '依個體狀況不同。一般原則：成體守宮拒食 14 天且體重明顯下降、或合併其他症狀（嘔吐、消瘦、糞便異常、神經症狀）就應就醫；幼體拒食超過 5–7 天應提早就醫。完整評估請使用本頁「緊急快篩」模式。'
      }
    },
    {
      '@type': 'Question',
      ...
      acceptedAnswer: {
        '@type': 'Answer',
        text: '本系統涵蓋的常見疾病包含：隱孢子蟲感染（最致命腸道寄生蟲）、球蟲感染、呼吸道感染、代謝性骨病（MBD）、口炎、腸阻塞、脫水、低溫燙傷等。每項都有對應的症狀清單與獸醫處置說明，可於評估結果或 DISEASES 知識庫查看。'
      }
    },
    {
      '@type': 'Question',
      ...
      acceptedAnswer: {
        '@type': 'Answer',
        text: '完整健康檢查模式完成後，會產生一份結構化的報告區塊，包含飼主觀察、各項目判讀與可能疾病比對。掛號特寵醫院前截圖此區直接給獸醫，可大幅縮短問診時間並減少資訊遺漏。'
      }
    },
    {
      '@type': 'Question',
      ...
      acceptedAnswer: {
        '@type': 'Answer',
        text: '不會。所有作答進度與結果僅儲存在你瀏覽器的 sessionStorage（本機），關閉分頁或瀏覽器後即清除。Gencko Breeding Studio 不會收集任何健康相關資料。'
      }
    }
  ]
}
  ...
  meta: [
    {
      name: 'description',
      content:
        '守宮健康評估系統：透過引導式問卷快速判讀守宮狀態。三種情境涵蓋緊急快篩（拒食、嘔吐、神經症狀等異常）、完整健康檢查、與購入前評估，並比對隱孢子蟲、球蟲、MBD、呼吸道感染等常見疾病症狀。本評估不取代專業獸醫診斷。'
    },
    {
      name: 'keywords',
      content:
        '守宮健康評估, 守宮拒食, 豹紋守宮生病, 守宮就醫指南, 隱孢子蟲, MBD, 守宮急診, 守宮症狀'
    },
    // Open Graph
    { property: 'og:title', content: '守宮健康評估系統｜緊急快篩、完整檢查、購入評估' },
    {
      property: 'og:description',
      content: '三種情境快速判讀守宮健康狀況。本評估僅供飼主自我檢視，不取代獸醫診斷。'
    },
...
watch(
  [mode, finished, () => answers.triage, () => answers.checkup, () => answers.purchase],
  saveState,
  { deep: true }
)
...
const triageVerdict = computed(() => {
  ...
  const isMaleBreedingFast =
    ...
    Array.isArray(ans.fastSymptoms) &&
    ans.fastSymptoms.includes('none')
...
const purchaseVerdict = computed(() => {
  ...
  if (hasCritical)
    return {
      ...VERDICTS.emergency,
      title: '這隻先不要買',
      headline: '照片已出現明顯健康紅旗，購入風險過高。',
      action: '只要看到眼睛分泌物、牙籤尾、潰瘍傷口或明顯無反應，直接換看下一隻。'
    }
  if (warnCount >= 1)
    return {
      ...VERDICTS.urgent,
      title: '先補看更多細節',
      headline: '目前有幾項需要再確認，單靠這組照片還不夠。',
      action: '請賣家補正面、側面、尾巴根部、眼部特寫，以及行走或進食影片後再決定。'
    }
  return {
    ...VERDICTS.healthy,
    title: '可列入考慮',
    headline: '從目前照片來看，外觀指標大致健康。',
    action: '購入前仍要逐項確認眼睛、尾巴、皮膚、體態與精神反應是否一致正常。'
  }
})
...
<template>
  <div
    :class="[
      'health-container',
      {
        'health-container--quiz': mode !== 'entry' && !finished,
  ...
  >
    <TheBackButton
      ...
      :fallback="mode === 'entry' ? '/' : null"
      :on-click="mode !== 'entry' ? exitToEntry : null"
    />
    ...
    <div v-if="mode === 'entry'" class="h-entry">
      ...
      <div class="h-card-grid">
        <button class="h-entry-card h-entry-card--urgent" @click="startMode('triage')">
          ...
          <div class="h-entry-desc">
            聚焦急性警訊、拒食、呼吸、外傷與環境異常，快速判斷是否要立即就醫。
          </div>
          <div class="h-entry-cta">
            開始快篩
            <span class="h-entry-cta-arrow">→</span>
          </div>
        </button>
        ...
        <button class="h-entry-card h-entry-card--full" @click="startMode('checkup')">
          ...
          <div class="h-entry-desc">
            涵蓋五官、體表、體態、排泄、活動與環境紀錄，適合做完整健康檢查。
          </div>
          <div class="h-entry-cta">
            開始檢查
            <span class="h-entry-cta-arrow">→</span>
        ...
        <button class="h-entry-card h-entry-card--purchase" @click="startMode('purchase')">
          ...
          <div class="h-entry-desc">
            只看購入前能確認的眼睛、體態、皮膚與精神反應，快速排除明顯風險。
          </div>
          <div class="h-entry-cta">
            開始評估
            <span class="h-entry-cta-arrow">→</span>
          ...
          <div class="h-q-options">
            ...
            >
              ...
              <div class="h-q-opt-check">
                <svg v-if="isSelected(q.id, opt.id)" viewBox="0 0 16 16" width="14" height="14">
                  <path
    ...
    <div v-else class="h-result">
      ...
      <div
        ...
        :style="{
          borderColor: verdictForReport.color,
          boxShadow: `0 8px 30px ${verdictForReport.color}33`
        }"
      >
        <div class="h-verdict-emoji">{{ verdictForReport.emoji }}</div>
        <div class="h-verdict-title" :style="{ color: verdictForReport.color }">
          {{ verdictForReport.title }}
        </div>
        <div class="h-verdict-headline">{{ verdictForReport.headline }}</div>
        <div class="h-verdict-action">{{ verdictForReport.action }}</div>

        <a v-if="showHospitalCta" href="/hospital" class="h-hospital-btn">🏥 查看爬蟲專科醫院 →</a>
      </div>

      <!-- Triage 模式：列出致命警訊 -->
      <div
        v-if="mode === 'triage' && triageCriticalItems.length"
        class="h-section h-section-danger"
      >
        <h3 class="h-section-title">🔴 觸發立即就醫的警訊</h3>
        <ul class="h-danger-list">
          <li v-for="(c, i) in triageCriticalItems" :key="i">
            <strong>{{ c.q.title.replace(/^Q\d+(-\d+)?\.\s*/, '') }}：</strong>
            {{ c.opt.label }}
          </li>
        </ul>
      </div>
      ...
      <div
        v-if="suspectedDiseases.length && mode !== 'purchase'"
        class="h-section h-section-disease"
      >
        <h3 class="h-section-title">📋 可能狀況</h3>
        <p class="h-disease-disclaimer">
          ⚠️ 僅供獸醫問診參考，非診斷依據，請以獸醫實際問診及診斷情況為主。
        </p>
        <div v-for="d in suspectedDiseases" :key="d.key" class="h-disease-card">
          ...
          <div class="h-disease-signs">
            <span class="h-disease-label">相關症狀：</span>
            {{ d.signs.join('、') }}
          </div>
      ...
      <div v-if="mode === 'checkup'" class="h-section">
        ...
        <div
          v-for="cat in [...new Set(checkupReport.map((r) => r.category))]"
          :key="cat"
          class="h-cat-block"
        >
          <div class="h-cat-title">{{ cat }}</div>
          <div
            v-for="(r, i) in checkupReport.filter((x) => x.category === cat)"
            :key="i"
            class="h-item-row"
            :class="'h-sev-' + r.severity + '-row'"
          >
      ...
      <div v-if="mode !== 'purchase'" class="h-section h-section-report" id="health-report-area">
        ...
        <div class="h-report-card">
          ...
          <div class="h-report-disclaimer">
            ※ 僅供獸醫問診參考，非診斷依據，請以獸醫實際問診及診斷情況為主。
          </div>
...
.health-container--quiz .h-back-btn {
  opacity: 1;
}
...
.h-q-card.is-highlighted {
  ...
  box-shadow:
    0 0 0 3px rgba(255, 69, 0, 0.14),
    0 10px 28px rgba(255, 69, 0, 0.14);
...
@keyframes questionHighlightPulse {
  ...
  50% {
    ...
    box-shadow:
      0 0 0 8px rgba(255, 69, 0, 0.08),
      0 12px 30px rgba(255, 69, 0, 0.18);
  }
  100% {
    border-color: var(--pri);
    background: rgba(255, 69, 0, 0.05);
...
.h-submit-btn:hover {
  ...
  box-shadow: 0 10px 24px var(--pri-glow);
}
.h-submit-btn.is-disabled {
  opacity: 0.35;
  box-shadow: none;
  background: var(--bd);
}
.h-submit-btn.is-disabled:hover {
  transform: none;
  box-shadow: none;
}
...
.h-report-table tbody tr:hover td {
  background: rgba(128, 128, 128, 0.04);
}
.h-report-table tbody tr:last-child td {
  border-bottom: none;
}
.h-sev-pill {
  display: inline-flex;
