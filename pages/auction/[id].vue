  ...
  if (currentAuction.value) {
    ...
    const product = {
      ...
      additionalProperty: [
        ...
        ...(a.birth_year
          ? [{ '@type': 'PropertyValue', name: '孵化日', value: a.birth_year }]
          : []),
        ...(a.diet ? [{ '@type': 'PropertyValue', name: '飲食', value: a.diet }] : []),
        ...(a.feeding_freq
          ? [{ '@type': 'PropertyValue', name: '餵食頻率', value: a.feeding_freq }]
          : []),
    ...
    const breadcrumb = {
      ...
      itemListElement: [
        { '@type': 'ListItem', position: 1, name: '首頁', item: 'https://www.genckobreeding.com/' },
        {
          ...
          position: 2,
          name: '守宮競標',
          item: 'https://www.genckobreeding.com/auction'
        },
...
useHead({
  ...
  meta: [
    ...
    // 已結標的競標 → noindex（避免過期內容污染索引）
    {
      name: 'robots',
      content: computed(() =>
        siteData.value.isEnded ? 'noindex, follow' : 'index, follow, max-image-preview:large'
      )
    },
    // Open Graph
    ...
    { property: 'og:type', content: 'product' },
    {
      ...
      content: computed(() => String(siteData.value.currentPrice || ''))
    },
    { property: 'product:price:currency', content: 'TWD' },
    {
      property: 'product:availability',
      content: computed(() => (siteData.value.isEnded ? 'out of stock' : 'in stock'))
    },
    ...
    { name: 'twitter:label1', content: '目前出價' },
    {
      name: 'twitter:data1',
      content: computed(() =>
        siteData.value.currentPrice ? `NT$ ${siteData.value.currentPrice}` : '尚無出價'
      )
    },
    {
      name: 'twitter:label2',
      content: computed(() => (siteData.value.isEnded ? '狀態' : '結標時間'))
    },
    {
      name: 'twitter:data2',
      content: computed(() =>
        siteData.value.isEnded
          ? '已結標'
          : siteData.value.endIso
            ? new Date(siteData.value.endIso).toLocaleString('zh-TW', { hour12: false })
            : ''
      )
    }
  ],
...
const generatePromo = async () => {
  ...
  try {
    ...
    const targetUrl =
      currentAuction.value.images && currentAuction.value.images.length > 0
    ...
    const statusTxt =
      getAuctionStatus(currentAuction.value).status === 'active' ? '🔥 競標中' : '🚫 已結標'
    const genderTxt =
      currentAuction.value.gender && currentAuction.value.gender !== '未定'
        ? `(${currentAuction.value.gender})`
        : ''
...
<template>
  <div class="auction-page-wrapper">
    ...
    <h1 v-if="currentAuction" class="sr-only">
      ...
      <template v-if="currentAuction.gender && currentAuction.gender !== '未定'">
      ...
      <div class="left-col">
        ...
        >
          ...
          <img
            :src="
              currentAuction.images && currentAuction.images.length
                ? getCleanUrl(currentAuction.images[0])
                : 'https://cdn.jsdelivr.net/gh/zzes50708/gencko-assets@main/img/placeholder.jpg'
            "
      ...
      <div class="right-col">
            ...
            <template v-if="store.currentUser">
              ...
              <div class="input-group">
                ...
                <input
                  ...
                  v-model="myBidAmount"
                  :min="minNextBid"
                  :step="currentAuction.min_increment"
                  aria-label="出價金額"
                />
              </div>
        ...
        <div class="info-section">
          <ul class="specs-list">
            ...
            <li>
              <strong>生日：</strong>
              {{ currentAuction.birth_year || '-' }}
            </li>
            <li>
              <strong>夾餵：</strong>
              {{ currentAuction.tongs_fed || '-' }}
            </li>
            <li>
              <strong>餌料：</strong>
              {{ currentAuction.diet || '-' }}
