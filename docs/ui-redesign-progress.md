...
- 文章載入統一為 Store `loadArticles`，SSR 與 hydration 共用資料、同時請求去重、錯誤可重試且獨立於其他次要資料；列表只取得已發布摘要，文章內頁再查正文，避免摘要快取被誤作完整文章。實際唯讀查詢同為 12 篇，JSON 大小 96,145 → 6,040 bytes，減少 93.7%，不是整頁速度測量。實測發現 articles 沒有 created_at 欄位，摘要查詢已依實際 schema 使用 publish_date。
...
- 使用者重新授權完成全部現有 UI，優先修正文章列表，全面排查桌機及手機。此段取代下面歷史紀錄的「其他官網收尾暫停」；純文字修訂仍由使用者下次另開，訂製爬櫃維持另案。
- `/articles` 根因：通用商品卡、首篇跨欄、固定圖高及正方形容器多套規則互相覆蓋。改用文章專屬類名與單一樣式來源，桌機三欄、平板兩欄、窄手機一欄，16:10 圖片，標題完整換行、分類標籤不再拉長，載入骨架同步比例與無外框樣式。搜尋、分類、資料與 SEO 不改。
- `/about` 觸控版輪播寬度、卡片尺寸被後置桌機規則覆蓋，正文變成窄欄且 CTA 被卡片遮擋。觸控正文改為置中全寬、以透明度切換，資訊卡獨立於下方排列；短手機保留可點 CTA，桌機立體輪播不改。降低動態效果設定停用資訊卡位移及掃描。
- `/profile` 修正已登入進頁立即 watcher 呼叫尚未初始化函式，以及切換帳號後舊請求覆蓋新結果；`/identity/:id` 列印只保留證書，排除導覽、Footer、浮動操作，離頁解除列印限定類名。
- 骨幹卡片八頁素材同步重擷取至 `public/previews/20260911-ui/`，避免展示舊文章跑版畫面；保留桌機清晰影片、手機照片、既有根網址觸控動畫及 `/care` 手勢。
- 全站 31 個路由／狀態 × 320、390、768、1024、1440、1920px 共 186 組均無整頁水平溢位。首輪 404 的六筆錯誤來自測試 initScript 在受限制文件存取 localStorage，補上測試防護後六組重測無錯；其餘首輪無 pageerror。文章及 About 的修正另做尺寸、內容位置與短手機按鈕遮擋驗證，不把可滑動比較表或跑馬燈當作頁面溢位。
- 證據：`output/ui-responsive-20260911/` 的 layout.json、rechecks.json、dark-recheck.json、articles-fixed-_.png、about-cards-_.png 與 identity-print.pdf。Typecheck、99/99 單元測試通過；本批 26 項 E2E 均已通過（分批執行）。首輪兩項失敗分別是素材版本斷言仍指向舊路徑、About 測試在 observer 掛載前按鍵，更新斷言及等待初始化後重測通過；最後 reduced-motion 補驗亦通過。手機手勢採瀏覽器原生觸控模擬，會員資料使用隔離 fixture，不宣稱實體手機或真實 OAuth 登入驗收。
- 目前變更留在 working tree，未部署／commit／push，保留既有檔案與本機 dev 3000；不與開發服務同時執行 Nuxt build。
- 下一個精確動作：等待使用者檢視本機全站 UI；如要求發布，再部署本批變更。純文字只在使用者明確開啟後續作，訂製爬櫃另案。測試與素材擷取程序均已結束，既有 localhost:3000 開發服務保留。

## 最新更新：骨幹卡片新版清晰素材（2026-09-11）

- 使用者要求根網址骨幹卡片影片更新成新版、不模糊，手機至少為照片。已從目前 working tree 的八個頁面重新擷取：care、articles、why-gencko、breeders、shop、genes、calculator、hospital。
- 版本化素材放在 `public/previews/20260911/`：桌機共用影片圖集 3840×1080（每格 960×540），展覽使用各頁 1280×720 獨立影片；手機卡面使用 3840×1080 照片圖集，展開使用各頁 1280×720 WebP，不建立展覽影片或額外轉場 Canvas。
- 移除卡面九點模糊取樣與重度染色，限制白底卡面亮度避免 Bloom 過曝，保留骨幹光效、原有旋轉與觸控時間軸。桌機影片失敗改用新版照片，不回退舊示範影片。
- Typecheck、99/99 單元測試通過；桌機影片及手機照片的載入尺寸、卡片開關通過，手機請求中沒有卡片 MP4。證據於 `output/hero-previews-20260911/`，擷取腳本 `scripts/generate-hero-previews.mjs`，既有素材及本機輸出均保留。
- 已部署 `277313d9484828a5769a8c0fd3cc83a6459e2582`；Production `6384234971`、Vercel `Aj14Ym2Tbb7mbAfqDSRnJnbrUFV3` 成功。正式桌機載入新版 atlas.mp4（206）與照片（200），手機僅載入新版 atlas.webp（200），兩種裝置無 pageerror。正式證據：`output/hero-previews-20260911/live-desktop.png`、`live-mobile.png`。手機原生觸控推進及返回首頁回歸亦通過。
- 下一個精確動作：等待使用者檢視根網址新版卡片素材；純文字、其他官網收尾維持暫停，訂製爬櫃另案。部署結果 checkpoint 留在本機，不為紀錄再次發布。
...
- 使用者追加 `/care` 手機可滑動模型，兩頁改好直接部署。已開放手機 WebGL、單指旋轉、雙指縮放；DPR 1、按需渲染、離屏卸載、WebGL 失敗示意圖保留。操作提示明確區分模型內手勢與區塊外捲頁。
...
- 已部署 `6fba18ebca161af09ac735c2d29526f0926569a5`，Production `6373368255`、Vercel `Ad5Fw1gQC9FHBrxuuMsVtMk5sNDn` 成功。正式 `/` 原生觸控推進、`/care` 單指旋轉與雙指縮放通過，HTTP 200 且無 pageerror；正式截圖位於 `output/hero-mobile-restored/live-root.png` 與 `live-care.png`。此為瀏覽器原生觸控模擬，不宣稱已操作使用者實體手機。
- 訂製爬櫃經使用者再次確認維持另案；純文字暫停。下一個精確動作：等待使用者確認手機動畫修復；其他官網收尾保持暫停，恢復後先處理上述會員 watcher 問題。部署結果 checkpoint 留於本機，不為紀錄再次觸發發布。
