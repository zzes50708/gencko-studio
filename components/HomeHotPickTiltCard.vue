...
<template>
  <AtroposCard :rotate="12" :highlight="false" :shadow="false" :lazy-init="true" :base-rotate-y="14" :base-rotate-x="2">
    <div class="hot-card-item hot-card-tilt">
      <NuxtLink :to="`/product/${item.ID}`" style="display:block; text-decoration:none; color:inherit; height:100%;">
        <div style="position:relative;">
          ...
          <img
            ...
            decoding="async"
          />
          <div v-else class="card-img" style="display:flex;align-items:center;justify-content:center;color:#666;font-size:0.95rem;background:#1a1a1a;">無圖片</div>

          <div v-if="item.Status === 'Sold'" class="hot-stamp hot-stamp-sold">SOLD</div>
          <div v-else-if="item.Status === 'Auction' && isAuction" class="hot-stamp hot-stamp-auction">競標</div>
        </div>
